...
def get_graph(select):
    if select == "Allsite":
        fig = px.pie(
            ...
            values="class",
            names="Launch_Site",
            title="Success Rate by Launch Sites",
...
def get_graf(select, Range):
    low, high = Range
    scatt_data = spacex_df[
        spacex_df["Payload_Mass_(kg)"].apply(lambda x: x > low and x < high)
    ].reset_index()
    if select == "Allsite":
        scatt_fig = px.scatter(
            scatt_data,
            x="Payload_Mass_(kg)",
            y="class",
        ...
        scatt_data = (
            scatt_data.groupby(["Launch_Site", "class", "Booster_Version"])[
                "Payload_Mass_(kg)"
            ]
            .sum()
            .reset_index()
